...
# Creating time series of years to estimate population size 
...
# Populating first row with year 0 populationsizes 
proj_year[1, ] <- fish_year0

# Create for loop that estimates the projected population based on the previous year  
for (i in 2:length(time_yr)) { #start with year 1 
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,]
    ...
    #to check for loop:
  #i <-- 2
  # project_year[2, ] <- fish_leslie %*% proj_year[2, -1] 
  #check matrix vector 
